...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    suggestedStatus: {
      ...
      nullable: true,
      description:
        `The current stage of this conversation. Check these rules IN ORDER and use the first one that matches: ` +
        ...
        `(2) "Quote Given" — our side HAS given a specific price/number at some point in the conversation. ` +
        `(3) "Follow-up Needed" — we already gave a price or info, and after that the LEAD (not us) went quiet/stopped responding, so WE need to check back in with THEM. ` +
        `(4) "Info Given" — our side shared product details/specs, but no price was asked for or given yet. ` +
        `(5) "First Message Sent" — our side sent a first reply/greeting only, no specifics yet. ` +
        `(6) "New" — the lead messaged but our business has not replied at all yet. ` +
