...
// There are two ways to reach Gemini here:
//   "vertex" — Vertex AI, authenticated with Google Cloud credentials (local ADC
//              via `gcloud auth application-default login`, or a service account
//              key). No API key or per-day cap, but it needs cloud credentials
//              present on the machine.
//   "api"    — the Gemini Developer API with a plain GEMINI_API_KEY (free tier is
//              ~1,500 requests/day). Nothing to authenticate beyond the key, so
//              it's what a deployed host uses.
// GEMINI_MODE chooses explicitly. With it unset, a deployed build (production)
// uses the API key and local dev uses Vertex — and `npm run dev` asks which one
// you want, setting GEMINI_MODE for that session.
export function geminiMode() {
  const explicit = (process.env.GEMINI_MODE || "").trim().toLowerCase();
  if (explicit === "api" || explicit === "vertex") return explicit;
  if (explicit) {
    throw new Error(`Invalid GEMINI_MODE "${explicit}" — use "api" or "vertex".`);
  }
  return process.env.NODE_ENV === "production" ? "api" : "vertex";
}
...
function getClient() {
  ...
  if (mode === "api") {
    ...
    if (!apiKey) {
      throw new Error(
        "GEMINI_API_KEY is not set — it's required in Gemini API mode. Add the key to " +
          "the environment, or set GEMINI_MODE=vertex to use Vertex AI instead."
      );
    }
    ...
    if (!project) {
      throw new Error(
        "GOOGLE_CLOUD_PROJECT is not set — it's required in Vertex AI mode. Set " +
          "GEMINI_MODE=api with a GEMINI_API_KEY to use the Gemini API instead."
      );
    }
  ...
  console.log(
    `[gemini] ${mode === "api" ? "Gemini API key" : "Vertex AI"} mode · model ${MODEL}`
  );
  return client;
}
...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    suggestedTime: {
      ...
      nullable: true,
      description:
        "The clock time of the lead's FIRST message in this conversation, as 24-hour HH:MM (e.g. \"09:04\", \"20:32\"). Take it exactly as displayed next to that message — do NOT convert or adjust for any timezone, and do not invent one. Null if no time is visible for that message.",
    },
    ...
    suggestedLastMessageTime: {
      type: Type.STRING,
      nullable: true,
      description:
        "The clock time of that LAST message, as 24-hour HH:MM. Take it exactly as displayed — do NOT convert or adjust for any timezone, and do not invent one. Null if no time is visible for that message.",
    },
    suggestedIdentified: {
      type: Type.STRING,
      nullable: true,
    ...
    suggestedLeadQuality: {
      ...
      nullable: true,
      description:
        `How promising this lead looks, judged ONLY on the LEAD's own messages (ignore how much WE said or offered — our eagerness doesn't make the lead better). Check "Unqualified" FIRST, before weighing Best/Medium/Poor. ` +
        `"Unqualified" — the lead is clearly not a real sales prospect: they explicitly say they're not interested / wrong number / just browsing and done, they're spam/bot/promotional content, a job or partnership pitch unrelated to buying jewelry, abusive, or the conversation shows this obviously isn't a genuine buyer. ` +
...
Read the conversation and return JSON matching the schema: a short description of what the lead wants (ignore our own sales pitch/boilerplate), the date AND clock time of the lead's first message in this conversation (YYYY-MM-DD and 24-hour HH:MM, or null if you can't tell), the date AND clock time of the LAST message in the conversation from either side (YYYY-MM-DD and HH:MM, or null), the lead's visible name/handle (or null), the product category being discussed, whether they asked about natural vs lab-grown, the conversation's current status/stage, any specific price we already quoted them, how promising the lead looks based on their own messages, and how far the conversation's back-and-forth structure has progressed. Leave any field null rather than guessing if it isn't clearly shown.`;
}

// Gemini is asked for 24-hour "HH:MM", but chat screenshots show times as
// "9:04 PM" just as often, so handle a 12-hour answer coming back rather than
// dropping the meridiem and silently storing a time 12 hours off. Anything
// still unparseable becomes null instead of a bad value.
...
export async function describeLead({ text, imageBase64, mimeType, todayISO }) {
  ...
  return {
    ...
    suggestedTime: normalizeTime(parsed.suggestedTime),
    suggestedLastMessageDate: parsed.suggestedLastMessageDate || null,
    suggestedLastMessageTime: normalizeTime(parsed.suggestedLastMessageTime),
