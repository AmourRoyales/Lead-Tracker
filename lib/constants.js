// Named here because both the status list and the conversation-stage list
// below key off it.
export const QUALITY_LEADS_STAGE = "Quality Leads";
...
// Parking status offered ONLY on leads sitting in the "Quality Leads" stage —
// a way to set one aside without losing it. Leads marked this way drop off the
// /quality-leads page. Deliberately kept out of STATUS_OPTIONS so it doesn't
// show up as a choice on every other lead.
export const CLOSED_FOR_NOW_STATUS = "Closed for now";

export function statusOptionsFor(leadQuality, conversationStage) {
  const base = GOOD_LEAD_QUALITIES.includes(leadQuality)
    ? [
        "New",
        "First Message Sent",
        "Info Given",
    ...
    : STATUS_OPTIONS;
  return conversationStage === QUALITY_LEADS_STAGE
    ? [...base, CLOSED_FOR_NOW_STATUS]
    : base;
}
