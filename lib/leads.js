...
export async function createLead(data) {
  ...
  const doc = {
    ...
    leadTime: data.leadTime || null, // "HH:MM" (24h) the lead first messaged | null
    ...
    lastMessageTime: data.lastMessageTime || null, // "HH:MM" (24h) | null
...
export async function updateLead(id, updates) {
  ...
  for (const key of allowed) {
    if (key in updates) {
      ...
      if (key === "leadTime" || key === "lastMessageTime") {
...
export async function listLeads(filters = {}) {
  ...
  // Used by /quality-leads to hide leads parked as "Closed for now". Combines
  // with an explicit status filter rather than overwriting it, so filtering
  // *for* the excluded status correctly returns nothing.
  if (filters.excludeStatus) {
    query.status = query.status
      ? { $eq: query.status, $ne: filters.excludeStatus }
      : { $ne: filters.excludeStatus };
  }
